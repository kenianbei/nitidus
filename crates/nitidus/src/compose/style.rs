//! Theming for the inline editor, and the line-styling pass that dims
//! quoted replies and signatures.
//!
//! The widget has no API for styling a range of text, so classification
//! is applied after it draws: each rendered row is mapped back to its
//! body line and recoloured. Only cells the editor left at the base style
//! are touched, so selection, search, and the cursor keep their own
//! styling — the precedence a decoration API would have given.
...
/// Applies the theme to a freshly created editor.
pub(super) fn apply(world: &World, area: &mut TextArea<'static>) {
    let Some(theme) = world.get_resource::<Theme>() else {
        return;
    };
    area.set_style(theme.base.default.normal.style());
    area.set_cursor_style(theme.base.default.selected.style());
...
/// Recolours the rows of `area` whose body line `styled` gives a style
/// for. `base` is the style the editor draws unstyled text in; a cell
