//! The fuzzy picker. Its entity spawns/despawns with the `ActiveOverlay`
//! resource (the vcard_tui pattern) and draws at `layer::OVERLAY`;
//! plurimus focus components track it, but keyboard input stays on the
//! router — resolved against the rebindable `picker` context, with
//! unbound printables typing into the filter (so global bindings never
...
pub(crate) fn select_row(world: &mut World, row: usize) {
    let mut overlay = world.resource_mut::<ActiveOverlay>();
    let Some(picker) = overlay.0.as_mut() else {
        return;
    };
