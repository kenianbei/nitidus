...
fn columns(sidebar_visible: bool) -> Vec<ColumnSpec> {
...
pub fn mail_layout(pane: MailPane, sidebar_visible: bool) -> LayoutFn {
    column_layout(columns(sidebar_visible), pane.column(sidebar_visible))
}
...
fn refresh_rules(
    ...
    if !(theme.is_changed() || tabs.is_changed() || sidebar.is_changed()) {
        return Ok(());
    }
...
fn render_rules(frame: &mut ratatui::Frame, area: Rect, state: &mut RulesWindow) -> Result {
    ...
    for gutter in layout::split_columns(area, &columns(state.sidebar_visible)).separators {
...
mod tests {
    ...
    fn rect(pane: MailPane, sidebar_visible: bool, area: Rect) -> Rect {
        mail_layout(pane, sidebar_visible)(&area)
    }
    ...
    fn a_rule_sits_in_every_seam_between_visible_panes() {
        ...
        let layout = layout::split_columns(area, &columns(true));
    ...
    fn a_collapsed_pane_takes_its_rule_with_it() {
        ...
        let layout = layout::split_columns(Rect::new(0, 0, 20, 20), &columns(true));
