//! Transport for the IMAP session: plain TCP or rustls TLS behind one
//! `AsyncRead + AsyncWrite` type, so the pump never cares which.
...
pub async fn upgrade_tls(tcp: TcpStream, host: &str) -> Result<ImapStream, MailError> {
